...
public class UpsertCompiler {
    private static void setValues(byte[][] values, int[] pkSlotIndex, int[] columnIndexes, PTable table, Map<ImmutableBytesPtr,RowMutationState> mutation, PhoenixStatement statement, boolean useServerTimestamp, Expression compare) {
        ...
        mutation.put(ptr, new RowMutationState(columnValues, statement.getConnection().getStatementExecutionCounter(), rowTsColInfo, compare));
    }
    ...
    private static MutationState upsertSelect(StatementContext childContext, TableRef tableRef, RowProjector projector,
        ...
        try (ResultSet rs = new PhoenixResultSet(iterator, projector, childContext)) {
            ...
            while (rs.next()) {
                ...
                setValues(values, pkSlotIndexes, columnIndexes, table, mutation, statement, useServerTimestamp, null);
                rowCount++;
    ...
    public MutationPlan compile(UpsertStatement upsert) throws SQLException {
            ...
            if (compareClause.getChildren().size() > 0) {
                ...
                if (lhs instanceof RowKeyColumnExpression) {
                    throw new SQLException("Use of row key in compare clause in currently not supported");
                       ...
                       && ((LiteralExpression)compareClause).getValue().equals(false)) {
        ...
        return new MutationPlan() {
            ...
            public MutationState execute() throws SQLException {
                ...
                setValues(values, pkSlotIndexes, columnIndexes, table, mutation, statement, useServerTimestamp, compare);
                return new MutationState(tableRef, mutation, 0, maxSize, connection, compare);
            }
